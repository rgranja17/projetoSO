...
char* __status_get_server_state(){
    ...
    ssize_t total_length = strlen(executing_tasks_str) + strlen(schedule_tasks_str) + strlen(executed_tasks_str) + 50; // Adicionando espaço extra para garantir
    char* status = (char*)malloc(total_length * sizeof(char)); // Alocando memória dinamicamente
    ...
    free(executing_tasks_str); // Liberando memória alocada dinamicamente
    free(schedule_tasks_str); // Liberando memória alocada dinamicamente
    free(executed_tasks_str); // Liberando memória alocada dinamicamente

...
int main(int argc, char** argv) {
    ...
    while (1) { // ciclo para a opção -u
        ...
        if(strcmp(tarefa_execute.comando,"status") == 0){
            int server_fifo = open(FIFO_NAME,O_WRONLY);
            if(server_fifo == -1) {
                perror("Erro ao abrir o FIFO");
                return 1;
            }
            char status_message[4096] = "";
            char* pending_tasks = getPendingTasks(queue,parallel_tasks,waiting_tasks);
            strcat(status_message,"Tarefas pendentes:\n");
            strcat(status_message,pending_tasks);

            getCompletedTasks(status_message);

            if(write(server_fifo,status_message,strlen(status_message))==-1) {
